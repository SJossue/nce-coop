...
and whether the existing "mass co-op approval automation system" (Registrar + Handshake data,
see `strategic-plan.md`) could extend to posting labeling. If either exists, it replaces
scraping entirely and sidesteps any ToS question. This is a five-minute question that could save
weeks of scraping work; see `questions.md`.
...
expired postings, and auto-applies the `rules.py` co-op-eligibility rule. This carries no
authentication risk beyond what he already has as a user: no scraping, no session automation.
...
access is acceptable, a distinct question from "can I build my own tooling," worth a direct yes
rather than an assumption. Only worth doing if the Phase 1 manual-paste cadence turns out to be
the actual bottleneck.

## Phase 3: sheet-ready export
Once the Bus Dev sheet's real columns are known (onboarding task 2, still pending), make
`export` produce a paste-ready CSV/markdown table matching them exactly, removing the manual
